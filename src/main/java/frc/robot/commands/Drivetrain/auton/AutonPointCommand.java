...
public class AutonPointCommand extends CommandBase {
      ...
      if (point < path.getLength() - 1) {
        double[] nearestPoint =
            getClosestPointOnLine(
                pathPoint.getX(),
                pathPoint.getY(),
                path.getPoint(point + 1).getX(),
                path.getPoint(point + 1).getY(),
                currentX,
                currentY);

        double velocityX =
            pathPoint.getVx()
                + (nearestPoint[0] - currentX) * Constants.Swerve.TRANSLATION_TUNING_CONSTANT;
        double velocityY =
            pathPoint.getVy()
  ...
  private double[] getClosestPointOnLine(
      double point1X,
      double point1Y,
      double point2X,
      double point2Y,
